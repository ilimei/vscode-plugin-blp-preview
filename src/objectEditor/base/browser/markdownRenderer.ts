...
import * as dompurify from '../browser/dompurify/dompurify';
import { DomEmitter } from '../browser/event';
import { createElement, FormattedTextRenderOptions } from '../browser/formattedTextRenderer';
...
export function renderMarkdown(markdown: IMarkdownString, options: MarkdownRenderOptions = {}, markedOptions: MarkedOptions = {}): { element: HTMLElement; dispose: () => void } {
	...
	markdownHtmlDoc.body.querySelectorAll('a')
		.forEach(a => {
			...
			if (
				...
				// drop the link
				a.replaceWith(...a.childNodes as any);
			} else {
	...
	if (codeBlocks.length > 0) {
		Promise.all(codeBlocks).then((tuples) => {
			...
			for (const placeholderElement of placeholderElements as any) {
	...
	if (options.asyncRenderCallback) {
		for (const img of element.getElementsByTagName('img') as any) {
